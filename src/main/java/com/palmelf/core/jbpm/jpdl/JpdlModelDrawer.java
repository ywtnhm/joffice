...
public class JpdlModelDrawer {
	...
	public BufferedImage draw(JpdlModel jpdlModel) throws IOException {
		...
		//标注当前活动节点
		this.drawCurrentNode(nodes,activityNames,g2, font);
		return bi;
	}


	private void drawCurrentNode(Map<String, Node> nodes, Set<String> activityNames,Graphics2D g2, Font font) {

		for (Node node : nodes.values()) {
			String name = node.getName();
			if (activityNames != null){
				for (String aName : activityNames){
					if (name != null && !"".equals(name) && name.equals(aName)){
						//当前任务节点
						int x = node.getX();
						int y = node.getY();
						int w = node.getWidth();
