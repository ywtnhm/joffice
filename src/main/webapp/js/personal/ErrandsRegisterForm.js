                ...
                if (d.getForm().isValid()) {
                    d.getForm().submit({
                        method: "post", waitMsg: "正在提交数据...", success: function (e, f) {
                            ...
                            Ext.getCmp("ErrandsRegisterGrid").getStore().reload();
                            b.close();
...
ErrandsRegisterForm.prototype.setup = function () {
    var a = new Ext.FormPanel({
        ...
        }, {name: "errandsRegister.flag", id: "flag", xtype: "hidden", value: 1}, {
            xtype: "hidden",
            name: "errandsRegister.userId",
            id: "userId"
        }, {xtype: "hidden", name: "errandsRegister.status", id: "status"}, {
            xtype: "hidden",
    ...
    if (this.dateId != null && this.dateId != "undefined") {
        a.getForm().load({
            ...
            success: function (e, f) {
                var b = f.result.data;
                var d = getDateFromFormat(b.startTime, "yyyy-MM-dd HH:mm:ss");
                var c = getDateFromFormat(b.endTime, "yyyy-MM-dd HH:mm:ss");
